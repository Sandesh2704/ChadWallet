...
// import type { Token, TokenTrade, TokenHolder, } from "@/types";
...
//     // Try BirdEye first - we need the token address from our mock data
//     // Since we're removing mock data, we'll need to search for the token first
//     const searchResults = await searchTokens(symbol);
//     const token = searchResults.find((t) => t.symbol.toLowerCase() === symbol.toLowerCase());
...
//   const data = await birdeyeFetch<{ tokens: Array<Record<string, unknown>> }>(
//     `/defi/token_trending?sort_by=rank&sort_type=asc&offset=0&limit=${limit}`
//   );

//   if (!data?.tokens) return MOCK_TOKENS.slice(0, limit);
...
//   const data = await birdeyeFetch<Record<string, unknown>>(`/defi/token_overview?address=${token.address}`);
//   if (!data) return token;

//   return {
//     ...token,
//     price: Number(data.price ?? token.price),
//     priceChange24h: Number(data.priceChange24hPercent ?? token.priceChange24h),
//     marketCap: Number(data.mc ?? token.marketCap),
//     volume24h: Number(data.v24hUSD ?? token.volume24h),
//     liquidity: Number(data.liquidity ?? token.liquidity),
//     holders: Number(data.holder ?? token.holders),
//   };
// }
...
//   if (!data?.items) return generateMockTrades(symbol, limit);
...
//   if (!data?.items) return generateMockHolders(token.price, limit);
...
//   if (!data?.items) return generateChartData(days, token.price);
